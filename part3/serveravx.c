            ...
            for(int ln = 0; ln < keysz; ln++){
                for(int col = 0; col < keysz; col++){
                    ...
                    for(int k = 0; k < nbytes; k+=8){
                        ...
                        cryp = _mm256_loadu_ps(&crypted[(istart + ln) * nbytes + k]);
                        vresult = _mm256_mul_ps(va, vb);
                        vresult = _mm256_add_ps(vresult, cryp);
                        _mm256_store_ps(&crypted[(istart + ln) * nbytes + k], vresult);
                    }
                }
            }
        }
        ...
        for(int i = 0; i < nr; i++){
            ...
            for(int ln = 0; ln < keysz; ln++){
                for(int col = 0; col < keysz; col++){
                    ...
                    for(int k = 0; k < nbytes; k+=8){
                        // printf("%d\n", crypted[(istart + ln) * nbytes + (k)]);
                        // printf("matmul at key[%d,%d] and file[%d,%d]. Writing in crypted[%d,%d]\n", ln, col, istart+col, k, istart + ln, k);
