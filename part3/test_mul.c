...
__m256 va, vb, vresult, cryp;

void line_mul_line_unrolled_avx(ARRAY_TYPE* file, ARRAY_TYPE* key, ARRAY_TYPE* crypt, int nbytes, int keysz){
    int nr = nbytes/keysz;
    for(int i = 0; i < nr; i++){
        int istart = i * keysz;
        for(int ln = 0; ln < keysz; ln++){
            for(int col = 0; col < keysz; col++){
                ARRAY_TYPE r = key[ln*keysz + col];
                for(int k = 0; k < nbytes; k+=8){
                    va = _mm256_loadu_ps(&file[(istart + col) * nbytes + k]);
                    vb = _mm256_set_ps(r,r,r,r,r,r,r,r);
                    cryp = _mm256_loadu_ps(&crypt[(istart + ln) * nbytes + k]);
                    vresult = _mm256_mul_ps(va, vb);
                    vresult = _mm256_add_ps(vresult, cryp);
                    _mm256_store_ps(&crypt[(istart + ln) * nbytes + k], vresult);
                }
            }
        }
    }
...
int main(int argc, char const *argv[])
{
    ...
    ARRAY_TYPE* key = aligned_alloc(64,sizeof(ARRAY_TYPE) * keysz*keysz);
    ...
    // generate file
    ARRAY_TYPE* file = aligned_alloc(64,sizeof(ARRAY_TYPE) * filesz * filesz);
    ...
    // allocate crypt
    ARRAY_TYPE* crypt = aligned_alloc(64, filesz*filesz*sizeof(ARRAY_TYPE));
    if(crypt == NULL){
    ...
    //with line_mul_line_unrolled_avx
    clock_t beg5 = clock();
    line_mul_line_unrolled_avx(file, key, crypt, filesz, keysz);
    clock_t finish5 = clock();
    double line5 = (double)(finish5-beg5)/CLOCKS_PER_SEC;
    printf("Execution time of line_mul_line_unrolled_avx : %f\n", line5);
    //print_matrix(crypt, filesz);
